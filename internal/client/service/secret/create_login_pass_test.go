...
func TestServiceCreateLoginPass(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cmd := &cobra.Command{}
			cmd.Flags().String("login", "user123", "")
			cmd.Flags().String("password", "pass123", "")
			...
			if tt.expectSecretCall {
				secretClient.EXPECT().
					Create(gomock.Any(), tt.tokenReturn, secret.LoginPassword, gomock.Any(), map[string]string{"key": "value"}).
					Return(&pb.SecretModel{Id: 1}, tt.secretErr)
			}

			s.CreateLoginPass(cmd, []string{})
		})
	}
}
