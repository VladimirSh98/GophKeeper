...
func TestServiceCreateText(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cmd := &cobra.Command{}
			cmd.Flags().String("text", "example text", "")
			...
			if tt.expectSecretCall {
				secretClient.EXPECT().
					Create(gomock.Any(), tt.tokenReturn, secret.TextData, gomock.Any(), map[string]string{"key": "value"}).
					Return(&pb.SecretModel{Id: 1}, tt.secretErr)
			}

			s.CreateText(cmd, []string{})
		})
	}
}
